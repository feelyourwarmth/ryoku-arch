...
// composes it, and a mismatch must name the library that moved: that line is
// what the Plugins page shows a user whose plugin stopped loading.
...
// bump: a receipt that matches wins over tier order, a copy with no receipt is
// trusted, and a set where every receipt disagrees is reported stale so the
// load can be left out instead of failing on every reload.
...
// hyprpm.toml is the recipe: the plugin tables in document order (skipping the
...
// the loaded guard keyed on the name the plugin reports. Nothing set, nothing
// emitted: the load alone is the block.
func TestPluginBlockConfigLua(t *testing.T) {
	...
	// the hyprbars buttons ride inside the same guard, after the config
...
// Turning a plugin off in the Hub must name it for an unload: Hyprland keeps a
// plugin its config stopped naming, so Save has to drop it explicitly.
func TestPluginsTurnedOff(t *testing.T) {
	prev := defaultOverrides()
	prev.Plugins.Hyprbars.Enabled = true
	prev.Plugins.Keysounds.Enabled = true
	prev.Plugins.Extra = map[string]ExtraPlugin{"hyprexpo": {Enabled: true}, "hyprtrails": {Enabled: true}}
	next := defaultOverrides()
	next.Plugins.Keysounds.Enabled = true
	next.Plugins.Extra = map[string]ExtraPlugin{"hyprtrails": {Enabled: true}}
	if got := strings.Join(pluginsTurnedOff(prev, next), ","); got != "hyprbars,hyprexpo" {
		t.Fatalf("turned off = %q", got)
	}
}
